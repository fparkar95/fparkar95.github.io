...
export class KeyboardComponent implements OnInit {
  ...
  constructor() {
    ...
    this.actualKeyObservable = fromEvent(document.body, 'keyup')
      .pipe(
        map((e: Event) => e = e as KeyboardEvent),
  ...
  consumeLetters(consumer : (letter : string) => any) : void {
    ...
    this.actualKeyObservable.subscribe((key) =>{
      if(!this.usedKeySet.has(key)){
         console.log(`Sending ${key} in guess`)
        this.newLetter.emit(key);
      }
    });
